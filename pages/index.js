			...
			<main className="md:max-w-6xl mx-auto ">
				...
				<section className="mt-5 px-4">
					<h2 className="text-xl md:text-2xl font-semibold mb-3">Aujourd'hui j'ai eu
						l'idée de le retaper d'<a className="underline"
					...
					<div className="sticky bottom-0 inset-x-0">
						<ShareLink
							...
							{link => <a
								...
								href={link} target="_blank">Partage le projet</a>}
						</ShareLink>
					</div>
				</section>
				<section className="mt-5 px-4">
					...
					<div className="flex flex-row flex-wrap gap-2">
						{sites && sites.map(s => <a className="underline font-semibold"
						                            key={s?.id}>{s?.title?.rendered}</a>)}
					</div>
				</section>
			</main>
			<footer className="px-6 my-12 text-center">
				<p className="font-semibold text-blue-500">Pondu avec humour par <a className="underline" href="https://marvinl.com" target="_blank">MarvinL.com</a></p>
			</footer>
		</div>
	)
}
